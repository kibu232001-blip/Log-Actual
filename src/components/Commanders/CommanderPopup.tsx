...
import { Commander, CommanderEvent, CDR_MAP } from '../../data/Commanders'
...
function Portrait({ style, color }: { style: string; color: string }) {
  ...
  return <div style={{ width:68, height:68 }} dangerouslySetInnerHTML={{ __html: svg }}/>
}
...
function TypeLine({ text, onDone }: { text:string; onDone:()=>void }) {
  ...
  useEffect(() => {
    setShown(''); let i=0
    const t = setInterval(() => { i++; setShown(text.slice(0,i)); if(i>=text.length){clearInterval(t);setTimeout(onDone,500)} }, 24)
...
export default function CommanderPopup({ event, onAction, onDismiss }: Props) {
  ...
  return (
    <div style={{ position:'fixed', bottom:80, right:340, width:420, zIndex:500, animation:'cdr-slide-in .4s cubic-bezier(.16,1,.3,1)' }}>
      <style>{`
        @keyframes cdr-slide-in{from{opacity:0;transform:translateX(40px)}to{opacity:1;transform:translateX(0)}}
      ...
      <div style={{
        ...
        {/* Header */}
        <div style={{ background:isEnemy?'rgba(40,6,6,.9)':'rgba(6,24,10,.9)', borderBottom:`1px solid ${bc}40`, padding:'7px 14px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            <div style={{ width:7, height:7, borderRadius:'50%', background:bc, boxShadow:`0 0 7px ${bc}`, animation:'cdr-blink .9s infinite' }}/>
            <span style={{ fontFamily:'Share Tech Mono,monospace', fontSize:14, letterSpacing:2, color:isEnemy?'#ff8866':'#2a8a5a' }}>
              {isEnemy ? '⚠ ENEMY CONTACT' : '▶ FRIENDLY COMMS'}
            </span>
          </div>
          <span style={{ fontFamily:'Share Tech Mono,monospace', fontSize:12, color:bc, letterSpacing:1 }}>{event.priority}</span>
        </div>
        ...
        <div style={{ padding:'14px 16px', display:'flex', gap:14, alignItems:'flex-start' }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ fontFamily:'Barlow Condensed,sans-serif', fontWeight:700, fontSize:24, color:bc, letterSpacing:1, marginBottom:1 }}>{cdr.name}</div>
            <div style={{ fontFamily:'Share Tech Mono,monospace', fontSize:12, color:`${bc}70`, letterSpacing:1, marginBottom:10 }}>{cdr.rank} — {cdr.role}</div>
            <div style={{ background:`${bc}08`, border:`1px solid ${bc}20`, borderRadius:'0 8px 8px 8px', padding:'10px 12px', position:'relative' }}>
              ...
              <p style={{ fontFamily:'Barlow,sans-serif', fontSize:20, lineHeight:1.75, color:isEnemy?'#ffcccc':'#c8e6c9', margin:0 }}>
        ...
        <div style={{ padding:'0 16px 6px', fontFamily:'Barlow Condensed,sans-serif', fontWeight:700, fontSize:16, letterSpacing:2, color:`${bc}55` }}>{event.title}</div>

        {allDone && (
          <div style={{ padding:'10px 16px 14px', borderTop:`1px solid ${bc}20`, display:'flex', gap:8 }}>
            {event.actionLabel && (
              <button onClick={()=>onAction(event)} style={{ flex:1, padding:'9px 0', background:`${bc}18`, border:`2px solid ${bc}`, color:bc, fontFamily:'Barlow Condensed,sans-serif', fontWeight:700, fontSize:18, letterSpacing:2, borderRadius:3, cursor:'pointer' }}>
                {event.actionLabel}
              </button>
            )}
            <button onClick={()=>onDismiss(event)} style={{ padding:'9px 14px', background:'transparent', border:'1px solid #1a3a20', color:'#2a5a3a', fontFamily:'Barlow Condensed,sans-serif', fontSize:16, borderRadius:3, cursor:'pointer' }}>
