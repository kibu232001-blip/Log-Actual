...
export default function SplashScreen({ onStart }: Props) {
  ...
  useEffect(() => {
    // Load and start music immediately — fades in over 1.5s
    const audio = new Audio('/audio/splash-music.mp3')
    audio.volume = 0
    audio.loop = false
    musicRef.current = audio

    audio.play().catch(() => {})  // May be blocked until user gesture

    // Fade in music over 1.5s
    let vol = 0
    ...
    <div onClick={handleClick} style={{
      ...
      <div style={{ position:'relative', zIndex:1, textAlign:'center', display:'flex', flexDirection:'column', alignItems:'center', gap:0 }}>
        ...
        <div style={{
          ...
          <div style={{
            fontSize:88, fontFamily:'Barlow Condensed, sans-serif', fontWeight:700,
            letterSpacing:8, color:'#2ecc71', lineHeight:1,
            textShadow:'0 0 40px rgba(46,204,113,0.3)',
          }}>LOG</div>
          <div style={{
            fontSize:132, fontFamily:'Barlow Condensed, sans-serif', fontWeight:700,
            letterSpacing:8, color:'#c8e6c9', lineHeight:1, marginTop:-8,
          }}>ACTUAL</div>
